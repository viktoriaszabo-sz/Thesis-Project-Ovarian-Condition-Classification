...
transform = transforms.Compose([
    ...
    transforms.Normalize([0.5], [0.5])  # Normalize (mean=0.5, std=0.5) for grayscale images
])
...
# Create DataLoaders
train_loader = DataLoader(dataset=train_dataset, batch_size=in_channels, shuffle=True)
test_loader = DataLoader(dataset=test_dataset, batch_size=in_channels, shuffle=False) 
...
class CNN(nn.Module): # this one is based on VGG-16 first to simplify the arcitecture at first (doubling in each layer) 
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1):
        ...
        self.conv1 = nn.Conv2d(in_channels, out_channels=in_channels*2, kernel_size=kernel_size, stride=stride) #convolving
        self.relu1 = nn.ReLU() #activation function
        self.pool1 = nn.MaxPool2d(kernel_size, stride=stride) # pooling

        self.conv2 = nn.Conv2d(in_channels*2, out_channels=in_channels*4, kernel_size=kernel_size, stride=stride) #convolving
        self.relu2 = nn.ReLU() #activation function
        self.pool2 = nn.MaxPool2d(kernel_size, stride=stride) # pooling

        self.conv3 = nn.Conv2d(in_channels*4, out_channels=in_channels*8, kernel_size=kernel_size, stride=stride) #convolving
        self.relu3 = nn.ReLU() #activation function
        self.pool3 = nn.MaxPool2d(kernel_size, stride=stride) # pooling
...
model = CNN(in_channels, out_channels, stride).to(device)
